...
times = []
accuracies = []
file = open("infoGPU.txt", "a")
for count in range(10, 500, 10):

    class Net(nn.Module):
        def __init__(self):
            super().__init__()
            self.conv1 = nn.Conv2d(3, count, 5).cuda()
            self.pool = nn.MaxPool2d(2, 2).cuda()
            self.conv2 = nn.Conv2d(count, 16, 5).cuda()
    ...
    if os.path.exists(PATH):
        ...
        print(f"Finished training, took {timeSince(start)}")
...
# plt.savefig("images/misclassificationGPU.eps")
...
# plt.savefig("images/confusion_matrixGPU.eps")
# print(end)
